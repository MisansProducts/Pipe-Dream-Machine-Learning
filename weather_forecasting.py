...
def run_training():
    print(f"\n{'#'*60}")
    print(f"# Weekly ML Training Started at {datetime.now()}")
    print(f"{'#'*60}")
    ...
    print(f"\n{'#'*60}")
    print(f"# Weekly ML Training Completed at {datetime.now()}")
    print(f"{'#'*60}\n")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if now.weekday() == 0 and now.hour == TRAINING_HOUR and now.minute == 0:
            print(f"\nMonday {TRAINING_HOUR}:00 AM detected - starting weekly training")
            try:
                run_training()
            except Exception as e:
                print(f"ERROR: Weekly training failed: {e}")
            time.sleep(60)
